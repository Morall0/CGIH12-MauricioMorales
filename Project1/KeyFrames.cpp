...
bool play = false; // Bandera para activar animacion
int playIndex = 0; // Se guarda el indice del frame actual
...
void saveFrame(void)
{
	...
	// Se guarda la informacion del frame
	KeyFrame[FrameIndex].dogPosX = dogPosX;
	KeyFrame[FrameIndex].dogPosY = dogPosY;
	KeyFrame[FrameIndex].dogPosZ = dogPosZ;

	KeyFrame[FrameIndex].rotDog = rotDog;
	KeyFrame[FrameIndex].head = head; // Se almacena el estado de la cabeza

	// Se cambia el indice al del siguiente frame
	FrameIndex++;
}
...
{
	// Frame siguiente menos frame actual entre el numero de pasos
...
void KeyCallback(GLFWwindow *window, int key, int scancode, int action, int mode)
{
	...
	if (keys[GLFW_KEY_L])
	{
		if (play == false && (FrameIndex > 1)) // Se verifica que no se este reproduciendo y que se tengan frames
		{
