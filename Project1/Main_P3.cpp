...
int main() {
 ...
 //     
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::rotate( model, 2.0f, glm::vec3( 1.0f, 0.0f, 0.0f ) ); // use to compare orthographic and perspective projection
		...
		// Actividad. Dibujar dos cubos extra a la derecha, con diferente traslación, escala, y rotación.

		model = glm::mat4(1); // Reiniciando el modelo

		model = glm::translate(model, glm::vec3( 5.0f, 0.0f, -5.0f ) ); // use with orthographic projection
		...
		model = glm::translate(model, glm::vec3(-5.0f, 0.0f, 0.0f)); // use with orthographic projection
		model = glm::rotate(model, 2.0f, glm::vec3(0.0f, 0.0f, 1.0f)); // use to compare orthographic and perspective projection
		model = glm::scale(model, glm::vec3(1.50f, 3.0f, 1.0f));
